...
curl --silent 'http://www.iatiregistry.org/api/search/dataset?filetype=organisation&all_fields=1&limit=4000' | jq '.results[] | .download_url' | sed 's/^["]*//;s/["]*$//' > sources/organisations

echo 'Retrieving list of activity sources from IATI Registry'
curl --silent 'http://www.iatiregistry.org/api/search/dataset?filetype=activity&all_fields=1&limit=4000' | jq '.results[] | .download_url' | sed 's/^["]*//;s/["]*$//' > sources/activities
